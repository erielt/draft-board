...
HttpRequest.prototype = {
   ...
   * Serve the draft URL.
   */
  serveDraftUrl: function(url) {
    console.log("serving draft url");
    var t = this;
    ...
    xhr.onprogress = function() {
      ...
      //t.write(this.response.slice(offset));
      console.log('written data: ' + this.responseText.substring(offset));
      t.write(this.responseText.substring(offset));

      offset = this.responseText.length; // byteLength for arraybuffer
    };

    xhr.onloadend = function() {
      console.log('loadend offset: ' + offset);
      console.log('loadend data: ' + this.responseText);
      t.end(this.responseText.substring(offset));
    };

    // TODO(eriel): need to get these from the extension.
    var noCache = Math.floor(Math.random() * 10000);
    var league = 176724;
    var slot = 1;
    var userId = 20621411;
    var token = "1:176724:1:20621411:1509386022";
    var baseUrl = "http://fantasydraft.espn.go.com/league-" + league + "/JOIN?";
    var params = "1=1&2=" + league + "&3=" + slot + "&4=" + userId + "&5=" + token + "&nocache=" + noCache + "&foo";
    console.log("requested URL: " + baseUrl + params);
    xhr.open('GET', baseUrl + params, true);
    xhr.send();
  },

  /*
   * Serve the config url.
   */
  serveConfigUrl: function(url) {
    console.log("serving config url");
    var t = this;
    ...
    xhr.onloadend = function() {
      ...
      var redacted = this.response;

      // Clear out the socketHost and socketPort so the draft app connects over 
      // http instead of flash sockets.
      redacted = redacted.replace(
        "<socketHost>fantasydraft.espn.go.com</socketHost>\n", "");
      redacted = redacted.replace("<socketPort>8080</socketPort>\n", "");

      var contentLength = 0;
      if (xhr.status == 200) {
        contentLength = (redacted && redacted.length) || 0;
      }
    ...
    // TODO(erielt): need to get these from the extension.
    var league = 176724;

    var baseUrl = "http://games.espn.go.com/ffl/draft/external/config?";
    var params = "seasonId=2015&leagueId=" + league 
      + "&draftTypeId=1&leagueSubTypeId=4&foo";
    console.log("requested URL: " + baseUrl + params);
    xhr.open('GET', baseUrl + params, true);
    xhr.send();
  },

  serveCrossdomainUrl: function(url) {
    console.log("Serving crossdomain url");
    ...
    var response = '<?xml version="1.0" encoding="UTF-8"?>\n'
      ...
      + '    <allow-access-from domain="*.usatoday.net" to-ports="*" secure="false"/>\n'
      ...
      + '    <allow-access-from domain="*.googlesyndication.com" to-ports="*" secure="false"/>\n'
      + '</cross-domain-policy>';
    ...
    this.writeHead(200, {
      ...
      'Date': 'Fri, 17 Jul 2015 02:27:04 GMT',
      'ETag': '"f57d9ddabc8fce1:0"',
      'Last-Modified': 'Fri, 02 Aug 2013 20:14:12 GMT',
      'P3P': 'CP="CAO DSP COR CURa ADMa DEVa TAIa PSAa PSDa IVAi IVDi CONi OUR SAMo OTRoBUS PHY ONL UNI PUR COM NAV INT DEM CNT STA PRE"',
      'Vary': 'Accept-Encoding',
      'X-N': 'S'});
